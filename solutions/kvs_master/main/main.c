...
static const char *TAG = "KVS_MASTER";
...
/* KVS signaling builds its own URLs from Kconfig region/channel settings.
 * The signal_url is only used as a non-NULL marker by esp_webrtc. */
static const char *signal_url = "kvs";

static int join_cmd(int argc, char **argv)
{
    start_webrtc((char *)signal_url);
    return 0;
}
...
static int init_console()
{
    ...
    esp_console_cmd_t cmds[] = {
        {
            .command = "join",
            .help = "Join the KVS channel (configured via menuconfig)\r\n",
            .func = join_cmd,
        },
        {
            .command = "leave",
            .help = "Leave from channel\n",
            .func = leave_room,
        },
        {
            .command = "i",
...
static int network_event_handler(bool connected)
{
    if (connected) {
        RUN_ASYNC(start, {
            ...
            webrtc_utils_wait_for_time_sync(20000);
            ESP_LOGI(TAG, "Joining KVS channel '%s' in region '%s'",
                     CONFIG_KVS_CHANNEL_NAME, CONFIG_KVS_AWS_REGION);
            if (start_webrtc((char *)signal_url) == 0) {
                ESP_LOGW(TAG, "Waiting for viewer to connect...");
            }
        });
    } else {
        stop_webrtc();
