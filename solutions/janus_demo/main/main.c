...
static int start_publish(int argc, char **argv)
{
    ...
    if (argc == 1) {
        // Use Janus settings
        start_webrtc(JANUS_SERVER, JANUS_ROOM_ID, JANUS_PIN, JANUS_TOKEN, JANUS_DISPLAY, JANUS_API_SECRET);
    } else {
        uint64_t room_id = argc > 2 ? strtoull(argv[2], NULL, 10) : JANUS_ROOM_ID;
        start_webrtc(argv[1], room_id, JANUS_PIN, JANUS_TOKEN, JANUS_DISPLAY, JANUS_API_SECRET);
    }
    return 0;
}
...
static int init_console()
{
    ...
    esp_console_cmd_t cmds[] = {
        {
            .command = "start",
            .help = "Start Janus VideoRoom publish\r\n",
            .func = start_publish,
        },
        {
            .command = "stop",
            .help = "Stop Janus VideoRoom publish\n",
            .func = stop_publish,
        },
        {
            .command = "i",
...
{
    if (connected) {
        RUN_ASYNC(start, {start_webrtc(JANUS_SERVER, JANUS_ROOM_ID, JANUS_PIN, JANUS_TOKEN, JANUS_DISPLAY, JANUS_API_SECRET);});
    } else {
        stop_webrtc();
    }
    return 0;
